    ...
    <section className="w-full bg-[#062B3D] text-white">
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-16 space-y-24">
        ...
        <Row
          ...
          title={
            <>
              Seeking <span className="text-[#2BB0ED]">Aligned Capital</span>{" "}
              Partners
            </>
          }
...
function Row({ image, label, title, desc, imageLeft }: RowProps) {
  return (
    <div className="grid lg:grid-cols-2 gap-10 items-center">
      {/* IMAGE */}
      <AnimatedWrapper
        type={imageLeft ? "slide-in" : "fade-in"}
      ...
      <AnimatedWrapper
        type="fade-up"
        delay={0.2}
        className={`space-y-4 ${imageLeft ? "lg:order-2" : "lg:order-1"}`}
      >
        <p className="text-sm text-gray-400">{label}</p>

        <h2 className="text-2xl font-semibold">{title}</h2>

        <p className="text-sm text-gray-300 leading-relaxed max-w-xl">{desc}</p>
      </AnimatedWrapper>
    </div>
  );
}

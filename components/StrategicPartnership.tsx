    ...
    <section className="bg-[#011627] text-white py-20 px-6 md:px-20 font-sans relative overflow-hidden">
      ...
      <div className="text-center mb-16 max-w-4xl mx-auto">
        ...
        <p className="text-slate-300 text-sm md:text-base leading-relaxed">
          We collaborate with leading institutions to create a comprehensive<br className="hidden md:block"/> 
        ...
        <div className="w-full lg:w-1/2 space-y-8">
          <div>
            ...
            <p className="text-slate-300 text-sm md:text-base leading-relaxed max-w-xl">
              From security operations centers to data analytics labs — across Nigeria, 
              Sierra Leone, the United States and beyond — Arise Funds portfolio 
              companies build the environments where real careers begin.
            </p>
          </div>
          ...
          <div className="space-y-6 pt-4">
            {categories.map((item, idx) => (
              <div key={idx} className="flex items-start gap-4">
                <div className={`${item.iconBg} p-2.5 rounded-full flex-shrink-0 shadow-lg`}>
                  {item.icon}
                </div>
                <div>
                  <h4 className="font-bold text-lg leading-tight">{item.title}</h4>
                  <p className="text-slate-400 text-xs md:text-sm mt-1">{item.description}</p>
                  <p className="text-slate-500 text-[10px] italic mt-0.5">{item.subtext}</p>
                </div>
              </div>
            ))}
          </div>
